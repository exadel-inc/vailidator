...
The server supports two interchangeable auditors, selected with the `AUDITOR` environment variable:
...
| `AUDITOR` | `claude` | Which agent runs audits: `claude` (Claude Agent SDK) or `native` (AI SDK ToolLoopAgent) |
| `LLM_BASE_URL` | `http://localhost:1234/v1` | Base URL for an OpenAI-compatible API (native auditor) |
| `LLM_API_KEY` | `local` | API key sent to the LLM provider (native auditor) |
| `LLM_MODEL` | `model-name` | Model name passed to the chat completions API (native auditor) |
| `ANTHROPIC_API_KEY` | *(unset)* | API key for the Claude auditor; omit to use the `claude login` subscription instead |
...
```bash
AUDITOR=native npm run dev   # AI SDK ToolLoopAgent + LLM_BASE_URL endpoint
...
src/
  ...
  agent/
    ...
    auditor-agent.ts             Native AI SDK ToolLoopAgent (OpenAI-compatible endpoint)
    ...
    system-prompt.ts             Shared system prompt for both auditors
  tools/
    lighthouse/lighthouse-runner.ts   Temporary HTML server and Lighthouse runner
    lighthouse/lighthouse-tool.ts     AI SDK tool wrapping the Lighthouse runner
    links-checker/links-checker.ts    Link checker implementation
    links-checker/links-checker-tool.ts AI SDK tool wrapping the link checker
